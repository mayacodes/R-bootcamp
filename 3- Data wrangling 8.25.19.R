...
ld.16y$subregion<-str_replace_all(ld.16y$subregion," parish","")
ld.16y %<>% mutate(log10cases=log10(1+all.cases))
state.map.ld.16y<-left_join(state_map,ld.16y)

map.ld.16y$avg_cases_state <- map.ld.16y %>% group_by(region) %>% summarise(avg_cases_state=sum(log10cases))

state.map.ld.16y$avg_cases_state <- state.map.ld.16y %>% group_by(region) %>% summarise(mean(log10cases))
...
county_map <- map_data("county")
state_map <- map_data("state")
map <-left_join(state_map,county_map)
p <- ggplot(data = map,
            aes(x = long, y = lat,
                group = group, fill = region))
...
county_map <- map_data("county")
state_map <- map_data("state")
map <-left_join(county_map, state_map)
p <- ggplot(data = map,
            aes(x = long, y = lat,
                group = group, fill = region))
